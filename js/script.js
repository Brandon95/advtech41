...
function create_particle(){
	...
	//subtract half the max, so some get negative numbers
...
function draw(){
	...
	//paint the canvas with a black rectangle that covers all circles make previously
	...
	for(var t=0; t<particles.length; t++){	
		...
		//which blends out to black at outer edge
		gradient.addColorStop(1, "black")
		ctx.fillStyle = gradient;
		//make the circle
		ctx.arc(p.x, p.y, p.radius, Math.PI*2,false);
		ctx.fill();
		//set up x,y so next time it move a bit
		p.x += p.vx;
		p.y += p.vy;
